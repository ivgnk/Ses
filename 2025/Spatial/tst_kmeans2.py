...
def main_part():
    ...
    for met in methods:
        ...
        pnt_old=['ok', 'oy', 'om']
        pn=['og', 'or', 'ob', ]
        plt.plot(w0[:, 0], w0[:, 1], pn[0], label='cluster 0', markersize=ms)  # ok - черный кружок
        plt.plot(w1[:, 0], w1[:, 1], pn[1], label='cluster 1', markersize=ms)  # oy - желтый кружок
        plt.plot(w2[:, 0], w2[:, 1], pn[2], label='cluster 2', markersize=ms)  # om - малиновый кружок
